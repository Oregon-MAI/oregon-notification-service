...
async def notifications(user_id: UUID, request: Request) -> StreamingResponse:
    """
    Возвращает SSE-поток уведомлений для пользователя
    :return: StreamingResponse с событиями в формате text/event-stream
    """
...
async def confirm(user_id: UUID, message_id: UUID) -> str:
    """
    Подтверждает прочтение уведомления и удаляет его из кэша
    :return: строка "success" или HTTPException с кодом 404
    """
